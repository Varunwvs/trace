...
$bid=0;
//batch info
$btinfo = $this->db->select('id, c_id, pid, p_code, batch_no, s_no_qty, mfd_date, exp_date')->from('pesticidebatch')->where(array('id'=> $id, 'c_id'=>$cid))->get();
...
//serial info
$sinfo = $this->db->select('count(*) as scount,sbsid, batch_id, serialno, cid, alias')->from('pesticidebatchserial')->where('batch_id', $brow->id)->get();
...
->from('pesticidebatch pb')
->join('pesticidebatchserial pbs', 'pbs.batch_id=pb.id')
