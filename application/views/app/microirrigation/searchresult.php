                  ...
                  <div class="col-12 text-sm"> 
                    ...
                    <?php if($srchcat==="upc"): ?>
                      ...
                      <?php $pinfo = $this->db->select('p_code, p_name, p_category, sub_category')
                            ->from('microirrigationproduct')->where(array('p_code'=>$srchterm, 'c_id'=>$comid))
                            ->get(); ?> 
                      ...
                      <?php foreach($pinfo->result() as $row): ?>
                        <?php $binfo = $this->db->select('id, pid, p_code, batch_no, s_no_qty, mfd_date, exp_date')
                                    ->from('microirrigationbatch')->where(array('p_code'=>$srchterm, 'c_id'=>$comid))
                                    ->get(); ?>
                      ...
                      <?php foreach($binfo->result() as $brow): ?>
                        <?php $cinfo = $this->db->select('ctid,pid, ucc, batchno, serialno, alias')
                                    ->from('microirrigationcontainer')->where(array('pid'=>$brow->pid, 'cid'=>$comid, 'batchno'=>$brow->id))
                                    ->get(); ?>
                    ...
                    <?php if($srchcat==="palias"): ?>
                      ...
                      <?php $painfo = $this->db->select('sb.id, sb.c_id, sb.pid, sb.p_code, sb.batch_no, sb.s_no_qty, 
                              sb.mfd_date, sb.exp_date, sbs.alias')->from('microirrigationbatch sb')
                              ->join('microirrigationbatchserial sbs', 'sbs.batch_id=sb.id', 'left')
                              ->where(array('sbs.alias'=>$sterm))->get();?>
                        <?php $pacount = $painfo->num_rows(); ?>
                        <?php if($pacount != '0'): ?>
                          <h3>Batch Details
                            <?php $sbsinfo = $this->db->select('sbsid, cid')->from('microirrigationbatchserial')->where(array('alias'=>$sterm, 'cid'=>$_SESSION['comid']))->get();?>
                            ...
                            <?php if($sbscount!=0):?>
                              ...
                              ?>
                            <?php endif ?>  
                            <a class="btn btn-primary btn-sm float-right" href="miserialqrcodeprint?id=<?php echo $sbsid ?>&cid=<?php echo $cid ?>" target="_blank"><i class="fa-solid fa-qrcode"></i> Generate / view full Label</a>                          
                          </h3>
                          ...
                          <?php foreach($painfo->result() as $brow): ?>
                            <?php $pinfo = $this->db->select('p_code, p_name, p_category, sub_category')
                            ->from('microirrigationproduct')->where(array('id'=>$brow->pid, 'c_id'=>$brow->c_id))
                            ->get(); ?> 
                    ...
                    <?php if($srchcat==="salias"): ?>
                      ...
                      <?php $painfo = $this->db->select('sb.id, sb.c_id, sb.pid, sb.p_code, sb.batch_no, sb.s_no_qty, 
                              sb.mfd_date, sb.exp_date, sc.alias')->from('microirrigationbatch sb')
                              ->join('microirrigationcontainer sc', 'sc.batchno=sb.id', 'left')
                              ->where(array('sc.alias'=>$sterm))->get();?>
                        <?php $pacount = $painfo->num_rows(); ?>
                        <?php if($pacount != '0'): ?>
                          <h3>Batch Details
                            <?php $sbsinfo = $this->db->select('ctid, cid')->from('microirrigationcontainer')->where(array('alias'=>$sterm, 'cid'=>$_SESSION['comid']))->get();?>
                            <?php $sbscount = $sbsinfo->num_rows(); ?>
                            <?php if($sbscount!=0):?>
                              <?php $ctid=''; $cid=''; ?>
                              <?php foreach($sbsinfo->result() as $sbsrow){
                              ...
                              ?>
                            <?php endif ?>  
                            <a class="btn btn-primary btn-sm float-right" href="micontainerqrcode?ctid=<?php echo $ctid ?>&cid=<?php echo $cid ?>" target="_blank"><i class="fa-solid fa-qrcode"></i> Generate / view full Label</a>                          
                          </h3>
                          <table class="table table-striped table-bordered nowrap rounded" style="width:100%;">
                            <thead class="bg-primary">
                              <tr>
