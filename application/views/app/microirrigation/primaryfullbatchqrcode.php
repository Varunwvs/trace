...
net_w, c_id, cml, cmlbis, licno')->from('microirrigationproduct')->where(array('id'=>$pid,'c_id'=>$cid))->get();
...
// for( ; $i < $scount ; $i++)
// {    
$srinfo = $this->db->select('serialno as srno')->from('microirrigationbatchserial')
->where('batch_id', $brow->id)->get();
foreach($srinfo->result_array() as $srrow){ 
    ...
    $pdf->SetXY($x,$y);
    $html = '<table border="0" cellpadding="5" width="100%" style="font-size:14px;">
            <tr>
            ...
            <tr>
                <td style="border-bottom: 1px solid black;"><h4>CML No. : '.$cml.'</h4></td>
                <td style="border-bottom: 1px solid black;"><h4>CM L BIS No : '.$cmlbis.'</h4></td> 
            </tr>
            <tr>
