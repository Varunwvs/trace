...
$btname = '';
//batch info
$btinfo = $this->db->select('id, c_id, pid, p_code, batch_no, s_no_qty')->from('microirrigationbatch2324')->where(array('id'=> $id, 'c_id'=>$cid))->get();
...
//product info
$pinfo = $this->db->select('name, marketed_by, itemcategory, subcategory, p_code, p_name, b_name, unit_w, net_w, c_id')->from('microirrigationproduct')->where(array('id'=>$pid,'c_id'=>$cid))->get();
...
//serial info
$sinfo = $this->db->select('count(*) as scount,sbsid, batch_id, serialno, cid, alias')->from('microirrigationbatchserial2324')->where('batch_id', $brow->id)->get();
...
$pdf->SetTitle('Financial Year 2023-2024 QRCodes');
$pdf->SetSubject('Financial Year 2023-2024 QRCodes');
...
// for( ; $i < $scount ; $i++)
// {       
$srinfo = $this->db->select('alias')->from('microirrigationbatchserial2324')->where('batch_id', $brow->id)->get();
...
$btname = date('d_m_Y', strtotime($batch_no));



// ob_end_clean();
// output the PDF document
$pdf->Output($btname.'_FY23_24labels.pdf', 'I');