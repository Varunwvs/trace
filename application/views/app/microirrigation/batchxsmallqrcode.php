...
$bid=0;
//batch info
$btinfo = $this->db->select('id, c_id, pid, p_code, batch_no, s_no_qty')->from('microirrigationbatch')->where(array('id'=> $id, 'c_id'=>$cid))->get();
...
//serial info
$sinfo = $this->db->select('count(*) as scount,sbsid, batch_id, serialno, cid, alias')->from('microirrigationbatchserial')->where('batch_id', $brow->id)->get();
...
// for( ; $i < $scount ; $i++)
// {       
$srinfo = $this->db->select('alias')->from('microirrigationbatchserial')->where('batch_id', $brow->id)->get();
