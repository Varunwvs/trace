...
$bid=0;
//batch info
$btinfo = $this->db->select('id, c_id, pid, p_code, batch_no, s_no_qty, mfd_date, exp_date')->from('fertilizerbatch')
...
->from('fertilizerproduct')->where(array('id'=>$pid,'c_id'=>$cid))->get();
...
//serial info
$sinfo = $this->db->select('count(*) as scount,sbsid, batch_id, serialno, cid, created_at')->from('fertilizerbatchserial')
...
$srinfo = $this->db->select('serialno as srno, alias')->from('fertilizerbatchserial')->where('batch_id', $brow->id)->get();
foreach($srinfo->result_array() as $srrow){ 
    ...
    $html = '<table border="0" cellpadding="3" width="100%" style="font-size:11px">
                <tr>
            ...
            ';
    $html.='</table>';
    $pdf->write2DBarcode($text, 'QRCODE,L', $x+117, $y+63, 22, 22, $style, '');
