...
//serial info
$sinfo = $this->db->select('count(*) as scount,sbsid, batch_id, serialno, cid')->from('fertilizerbatchserial')->where('batch_id', $brow->id)->get();
...
// for( ; $i < $scount ; $i++)
// {    
$srinfo = $this->db->select('serialno as srno')->from('fertilizerbatchserial')->where('batch_id', $brow->id)->get();
foreach($srinfo->result_array() as $srrow){ 
    ...
    $pdf->write2DBarcode($text, 'QRCODE,L', $x+110, $y-2, 45, 45, $style, 'N');
    
    $pdf->SetXY($x,$y);
    $html = '<table border="0" cellpadding="5" width="100%" style="font-size:10px;">
            <tr>
            ...
            <tr>
                ...
                <td style="border-color:white;"><h4>Serial No : '.$srrow['srno'].'</h4></td> 
            </tr>
            <tr>
