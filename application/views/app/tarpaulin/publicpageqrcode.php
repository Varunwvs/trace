...
$bid=0;
//batch info
$btinfo = $this->db->select('id, c_id, pid, p_code, batch_no, s_no_qty, mfd_date, exp_date')->from('tarpaulinbatch')->where(array('id'=> $id, 'c_id'=>$cid))->get();
...
//product info
$pinfo = $this->db->select('name, marketed_by, itemcategory, subcategory, p_code, p_name, b_name, unit_w, net_w, c_id, formulation, cibno, mlno')->from('tarpaulinproduct')->where(array('id'=>$pid,'c_id'=>$cid))->get();
...
//serial info
$sinfo = $this->db->select('count(*) as scount,sbsid, batch_id, serialno, cid, alias')->from('tarpaulinbatchserial')->where('batch_id', $brow->id)->get();
...
// for( ; $i < $scount ; $i++)
// {       
$srinfo = $this->db->select('pb.batch_no,pb.mfd_date,pb.exp_date,pb.pid,pbs.alias,pp.mrp')
->from('tarpaulinbatch pb')
->join('tarpaulinbatchserial pbs', 'pbs.batch_id=pb.id')
->join('productinfo pp', 'pp.pid=pb.pid')
...
foreach($srinfo->result_array() as $srrow){ 
    ...
    $text = base_url().'sb/'.$cid.'/'.$pid.'/'.$p_code.'/'.$bid.'?11='.$srrow['alias'].'&12='.$srrow['alias'];
    $x = $pdf->GetX();
    $y = $pdf->GetY();
    ...
    // set font
    $text2 = strtoupper($srrow['batch_no']).'<br>'.date('d/m/Y', strtotime($srrow['mfd_date'])).'<br>'.date('d/m/Y', strtotime($srrow['exp_date'])).'<br>'.$srrow['mrp'].'/-';
