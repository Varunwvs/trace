                  ...
                  <div class="col-12 text-sm"> 
                    ...
                    <?php if($srchcat==="upc"): ?>
                      ...
                      <?php $pinfo = $this->db->select('p_code, p_name, p_category, sub_category')
                            ->from('pesticideproduct')->where(array('p_code'=>$srchterm, 'c_id'=>$comid))
                            ->get(); ?> 
                      ...
                      <?php foreach($pinfo->result() as $row): ?>
                        <?php $binfo = $this->db->select('id, pid, p_code, batch_no, s_no_qty, mfd_date, exp_date')
                                    ->from('pesticidebatch')->where(array('p_code'=>$srchterm, 'c_id'=>$comid))
                                    ->get(); ?>
                      ...
                      <?php foreach($binfo->result() as $brow): ?>
                        <?php $cinfo = $this->db->select('ctid,pid, ucc, batchno, serialno, alias')
                                    ->from('pesticidecontainer')->where(array('pid'=>$brow->pid, 'cid'=>$comid, 'batchno'=>$brow->id))
                                    ->get(); ?>
                    ...
                    <?php if($srchcat==="palias"): ?>
                      ...
                      <?php $painfo = $this->db->select('sb.id, sb.c_id, sb.pid, sb.p_code, sb.batch_no, sb.s_no_qty, 
                              sb.mfd_date, sb.exp_date, sbs.alias')->from('pesticidebatch sb')
                              ->join('pesticidebatchserial sbs', 'sbs.batch_id=sb.id', 'left')
                              ->where(array('sbs.alias'=>$sterm))->get();?>
                        <?php $pacount = $painfo->num_rows(); ?>
                        <?php if($pacount != '0'): ?>
                          <h3>Batch Details
                            <?php $sbsinfo = $this->db->select('sbsid, cid')->from('pesticidebatchserial')->where(array('alias'=>$sterm, 'cid'=>$_SESSION['comid']))->get();?>
                          ...
                          <?php foreach($painfo->result() as $brow): ?>
                            <?php $pinfo = $this->db->select('p_code, p_name, p_category, sub_category')
                            ->from('pesticideproduct')->where(array('id'=>$brow->pid, 'c_id'=>$brow->c_id))
                            ->get(); ?> 
                    ...
                    <?php if($srchcat==="salias"): ?>
                      ...
                      <?php $painfo = $this->db->select('sb.id, sb.c_id, sb.pid, sb.p_code, sb.batch_no, sb.s_no_qty, 
                              sb.mfd_date, sb.exp_date, sc.alias')->from('pesticidebatch sb')
                              ->join('pesticidecontainer sc', 'sc.batchno=sb.id', 'left')
                              ->where(array('sc.alias'=>$sterm))->get();?>
                        <?php $pacount = $painfo->num_rows(); ?>
                        <?php if($pacount != '0'): ?>
                          <h3>Batch Details
                            <?php $sbsinfo = $this->db->select('ctid, cid')->from('pesticidecontainer')->where(array('alias'=>$sterm, 'cid'=>$_SESSION['comid']))->get();?>
                            <?php $sbscount = $sbsinfo->num_rows(); ?>
                            <?php if($sbscount!=0):?>
                              <?php $ctid=''; $cid=''; ?>
                              <?php foreach($sbsinfo->result() as $sbsrow){
