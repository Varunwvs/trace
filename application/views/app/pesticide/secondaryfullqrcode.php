...
$pdf->SetTitle('Secondary Label QR Code');
$pdf->SetSubject('Secondary Label QR Code');
...
$query = $this->db->query("SET SESSION group_concat_max_len = 2048");
 $sql="SELECT `sc`.`ctid`, `sc`.`pid`, `sc`.`ucc`, `sc`.`batchno`, `sc`.`serialno`, 
        ...
        WHERE `sc`.`cid` = $cid AND `sc`.`batchno` = $id GROUP BY `sc`.`alias` ORDER BY `sc`.`ucc` ASC";
...
$bnfo = $this->db->select('batch_no')->from('pesticidebatch')->where('id', $id)->get();
...
$pnfo = $this->db->select('p_name')->from('pesticideproduct')->where('id', $pid)->get();
