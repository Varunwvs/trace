...
//product info
$pinfo = $this->db->select('name, marketed_by, p_category, sub_category, p_code, p_name, b_name, unit_w, net_w, c_id')
...
//serial info
$sinfo = $this->db->select('count(*) as scount,sbsid, batch_id, serialno, cid')->from('seedbatchserial')->where('batch_id', $brow->id)->get();
...
$pdf->SetTitle('Full QR Code');
$pdf->SetSubject('Full QR Code');
...
// for( ; $i < $scount ; $i++)
// {    
$srinfo = $this->db->select('serialno as srno')->from('seedbatchserial')->where('batch_id', $brow->id)->get();
foreach($srinfo->result_array() as $srrow){ 
    ...
    $text = 'Product Name : '.$p_name.PHP_EOL.'Brand Name : '.$b_name.PHP_EOL.'UPC : '.$p_code.PHP_EOL.'Batch No : '.$batch_no.PHP_EOL.'Serial No : '.$srrow['srno'].PHP_EOL.'Mfg Date : '.$mfd_date.PHP_EOL.'Exp Date : '.$exp_date;
    $x = $pdf->GetX();
    $y = $pdf->GetY();
    $pdf->setCellMargins(0,0,0,0);
    $pdf->write2DBarcode($text, 'QRCODE,L', $x+110, $y, 27, 27, $style, 'N');
    
    $pdf->SetXY($x,$y);
    $html = '<table border="0" cellpadding="5" width="100%" style="font-size:12px">
            <tr>
