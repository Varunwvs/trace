...
$pinfo = $this->db->select('name, marketed_by, p_category, sub_category, p_code, p_name, b_name, unit_w, net_w, c_id, registration_no')->from('seedproduct')->where(array('id'=>$pid,'c_id'=>$cid))->get();
...
// for( ; $i < $scount ; $i++)
// {       
$srinfo = $this->db->select('pb.batch_no,pb.mfd_date,pb.exp_date,pb.pid,pbs.serialno,pbs.alias')
->from('seedbatch pb')
->join('seedbatchserial pbs', 'pbs.batch_id=pb.id')
...
foreach($srinfo->result_array() as $srrow){ 
    ...
    $text = base_url().'sb/'.$cid.'/'.$pid.'/'.$p_code.'/'.$bid.'/'.$srrow['alias'].'?11='.$srrow['serialno'];
    // $text = '12345';
    $x = $pdf->GetX();
    $y = $pdf->GetY();
