                            ...
                            if($ccount>0){
                                ...
                                foreach($cinfo->result() as $row){                                    
                                    $contentp1.= '<tr>
                                                    ...
                                                    if($row->category==='1'){
                                                        $lbl='seed';
                                                        $pqrqty = $this->db->select('*')->from('seedbatchserial2324')->where(array('cid'=>$row->id))->get();                                                        
                                                        $pqrcount = $pqrqty->num_rows();
                                                        $sqrqty = $this->db->select('*')->from('seedcontainer2324')->where(array('cid'=>$row->id))->get();
                                                        $sqrcount = $sqrqty->num_rows();
                                                        $qrcount = $pqrcount + $sqrcount;
                                                    }
                                                    elseif($row->category==='2' || $row->category==='6'){
                                                        $lbl='fertilizer';
                                                        $pqrqty = $this->db->select('*')->from('fertilizerbatchserial2324')->where(array('cid'=>$row->id))->get();                                                        
                                                        $pqrcount = $pqrqty->num_rows();
                                                        $sqrqty = $this->db->select('*')->from('fertilizercontainer2324')->where(array('cid'=>$row->id))->get();                                                        
                                                        $sqrcount = $sqrqty->num_rows();
                                                        $qrcount = $pqrcount+$sqrcount;
                                                    }
                                                    elseif($row->category==='3'){
                                                        $lbl='micro_irrigation';
                                                        $qrqty = $this->db->select('*')->from('microirrigationbatchserial2324')->where(array('cid'=>$row->id))->get();                                                        
                                                        $qrcount = $qrqty->num_rows();
                                                    }
                                                    elseif($row->category==='5'){
                                                        $lbl='pesticide';
                                                        $pqrqty = $this->db->select('*')->from('pesticidebatchserial2324')->where(array('cid'=>$row->id))->get();                                                        
                                                        $pqrcount = $pqrqty->num_rows();
                                                        $sqrqty = $this->db->select('*')->from('pesticidecontainer2324')->where(array('cid'=>$row->id))->get();                                                        
                                                        $sqrcount = $sqrqty->num_rows();
                                                        $qrcount = $pqrcount+$sqrcount;
                                                    }
                                                    elseif($row->category==='7'){
                                                        $lbl='agro_processing';
                                                        $qrqty = $this->db->select('*')->from('tarpaulinbatchserial2324')->where(array('cid'=>$row->id))->get();                                                        
                                                        $qrcount = $qrqty->num_rows();
                                                    }
                                                    else{
                                                        $qrcount = 0;
