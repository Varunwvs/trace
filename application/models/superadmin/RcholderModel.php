 ...
 * Rcholder Model
 */
class RcholderModel extends CI_Model
{
    ...
    function dtRcholder(){
        ...
        (select count(*) from seedbatch sb where sb.c_id=u.id) as bcount')
         ...
         ->where(array('u.role'=>'rcholder', 'u.ulevel'=>'3', 'u.category'=>'1'))
        ...
        foreach ($exp->result() as $row) {
            ...
            $actionButton = '
                ...
                <li class=" d-print-none"><a class="btn text-red btn-xs" role="button" id="update_rcstatus" data-id="'.$row->id.'" data-toggle="tooltip" title="Update RC Holder Status"> <span class="fa-solid fa-right-left"></span> Update RC Holder Status</a></li>
                <li class=" d-print-none"><a class="btn text-orange btn-xs" role="button" id="update_rcrole" data-id="'.$row->id.'" data-toggle="tooltip" title="Change role"> <span class="fa-solid fa-tent-arrow-left-right"></span> Change Role</a></li>
                ...
                <li class=""><a class="btn text-success btn-xs" role="button" id="view_cinfo" data-bs-toggle="modal" data-bs-target="#viewModal" data-toggle="tooltip" title="View RC Holder details" onclick="viewMember('.$row->id.')"> <span class="fa-regular fa-eye"></span> View RC Holder Details</a></li>
                <li class=""><a class="btn text-info btn-xs" role="button" href="rcholderedit?id='.$row->id.'" data-toggle="tooltip" title="Update company details"> <span class="fa fa-edit"></span> Update RC Holder Details</a></li>
                <li class=" d-print-none hidden"><a class="btn text-red btn-xs" role="button" id="delete_rcholder" data-id="'.$row->id.'" data-toggle="tooltip" title="Delete RC Holder"> <span class="fa fa-trash"></span> Delete RC Holder</a></li>
            ...
            $output[] = array(
                ...
                <a class="btn text-info btn-xs" role="button" id="view_ciinfo" data-bs-toggle="modal" data-bs-target="#viewinfoModal" data-toggle="tooltip" title="View rc holder details" onclick="viewiMember('.$row->id.')"> <span class="fa-regular fa-eye"></span> View More</a>',
                $cat,
                $status,
                $actionButton
            ); 
    ...
    function dtFlrcholder(){
        ...
        $length = intval($this->input->get("length"));
        $exp=$this->db->select('u.id, u.name, u.category, u.city, u.state, u.pincode, u.address, u.role, 
        u.status, u.totalproduct, u.qr_quantity, c.name as catname, (select count(*) from fertilizerproduct sp where sp.c_id=u.id) as pcount,
        ...
        where sbs.cid=u.id) as prcount, (select count(*) from fertilizercontainer sbc where sbc.cid=u.id) as srcount')
         ->from('users u')
         ->join('categories c', 'c.id=u.category', 'left')
         ->where(array('u.role'=>'rcholder', 'u.ulevel'=>'3'))
         ->group_start()
    ...
    function dtPsrcholder(){
        ...
        $length = intval($this->input->get("length"));
        $exp=$this->db->select('u.id, u.name, u.category, u.city, u.state, u.pincode, u.address, u.role, 
        u.status, u.totalproduct, u.qr_quantity, c.name as catname, (select count(*) from pesticideproduct sp where sp.c_id=u.id) as pcount,
    ...
    function dtMircholder(){
        ...
        $length = intval($this->input->get("length"));
        // $exp=$this->db->select('u.id, u.name, u.category, u.city, u.state, u.pincode, u.address, u.role, 
        // u.status, u.totalproduct, u.qr_quantity, c.name as catname, (select count(*) from microirrigationproduct sp where sp.c_id=u.id) as pcount,
    ...
    function incrcTotalcount(){
        $id=$this->input->post('member_id');
        $rctotalcount = $this->input->post('rctotalcount');
        $data = array('totalproduct' => $rctotalcount);
        $result = $this->db->update('users',$data,array('id'=>$id));
    ...
    function delRcholder(){
        ...
        if($result){
            $output['success'] = true;
            $output['messages'] = 'Successfully Removed RC Holder';
        }else{
            $output['success'] = false;
            $output['messages'] = 'Error while removing RC Holder!';
        }
    ...
    function updRcholder(){
        ...
        if($result){
            $output['success'] = true;
            $output['messages'] = 'Successfully Updated rcholder info';  
        }else{
            $output['success'] = false;
            $output['messages'] = 'Error While updating rcholder info';  
        }        
        return $output;
    }
}