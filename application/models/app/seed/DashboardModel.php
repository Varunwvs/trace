...
class DashboardModel extends CI_Model
{
    ...
    function prdCount(){
        if($_SESSION['level']==='3' && ($_SESSION['comcat']==="1" || $_SESSION['comcat']==="6") && 
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cp = $this->db->select('*')->from('seedproduct')
    ...
    function bthCount(){
        if($_SESSION['level']==='3' && ($_SESSION['comcat']==="1" || $_SESSION['comcat']==="6") && 
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cp = $this->db->select('*')->from('seedbatch')
            ->where(array('c_id'=> $_SESSION['comid'], 'status'=>'1'))->get();
            $query = $cp->num_rows();
        }else{            
            $cp = $this->db->select('*')->from('seedbatch')
    ...
    function cthCount(){
        if($_SESSION['level']==='3' && ($_SESSION['comcat']==="1" || $_SESSION['comcat']==="6") && 
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cp = $this->db->select('*')->from('seedbatchserial')
    ...
    function cthdCount(){
        if($_SESSION['level']==='3' && ($_SESSION['comcat']==="1" || $_SESSION['comcat']==="6") && 
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cp = $this->db->select('*')->from('seedcontainer')
            ->where(array('cid'=> $_SESSION['comid']))->get();
            $query = $cp->num_rows();
        }else{
            $cp = $this->db->select('*')->from('seedcontainer')
