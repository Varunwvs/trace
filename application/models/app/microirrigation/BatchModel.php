...
class BatchModel extends CI_Model
{
    public function __construct(){
        ...
        // Set table name
        $this->table = 'microirrigationbatch';
        // Set orderable column fields
        $this->column_order = array('batch_no', 's_no_qty', 'schedule_cron');
        // Set searchable column fields
        $this->column_search = array('batch_no', 's_no_qty', 'schedule_cron');        
        // Set default order
    ...
    public function get_maxalias(){
        ...
        $this->db->from('microirrigationbatchserial');
        $res = $this->db->get()->row();
    ...
    function addBatch(){
        ...
        // $mfgdate = date('Y-m-d', strtotime($this->input->post('mfgdate')));  
        $pid = $this->input->post('pid');
        $upc = $this->input->post('upc');
        $netw = $this->input->post('netw');
        $s_no = $this->input->post('s_no');
        $status = "1";
        $count = (int)$this->input->post('s_no');
        $this->db->trans_start();
        // $result = '';
        $data = array('c_id'=>$cid, 'pid'=>$pid, 'p_code'=>$upc, 'net_w'=>$netw, 's_no_qty'=>$s_no, 'status'=>$status);
        $result = $this->db->insert('microirrigationbatch',$data);
        ...
        if($datat){
            ...
            $results = $this->db->insert_batch('microirrigationbatchserial',$datat);
        }
    ...
    function updPrintstatus(){
        ...
        $print_status='0';
        $sinfo = $this->db->select('print_status')->from('microirrigationbatch')->where('id', $id)->get();
        ...
        $data = array('print_status' => $print_status);
        $result = $this->db->update('microirrigationbatch',$data,array('id'=>$id));
        if($result){
            $output['success'] = true;
    ...
    function getBatchinfo(){
        $id=$this->input->post('member_id');
        $pinfo = $this->db->select('id, c_id, pid, batch_no')
                ->from('microirrigationbatch')->where(array('id'=> $id, 'c_id' => $_SESSION['comid']))->get();
    ...
    function updBatch(){
        ...
        $result = $this->db->update('microirrigationbatch',$data,array('id'=>$edit_bid));
    ...
    public function get_batch_serial_no($id){
        ...
        ->from('microirrigationbatchserial')
        ->where('batch_id',$id)
        ->where('api_sent','0')
        ->get()->result();
        return $result;
    ...
    public function update_batch_api($sid){
        ...
        $this->db->update('microirrigationbatchserial',$data);
    }
    
    public function update_batch($bid){
        $data = array('api_sent'=>'1', 'schedule_cron'=>'2');
        $this->db->where('id',$bid);
        $this->db->update('microirrigationbatch',$data);
    }
    ...
    public function label_detail($sno){
       ...
       // $this->db->where('b.id','519');
        ...
        $this->db->join('microirrigationbatchserial bs','bs.batch_id = b.id');
        $this->db->from('pesticidebatch b');
        return $this->db->get()->result();
    }
    
    public function batchExport($id,$comid){
        $this->db->select('b.id as batchid,b.batch_no, p.p_name,p.p_code,p.itemcategory,p.subcategory');
        // ,,b.mfd_date,b.exp_date,b.id as batchid
        $this->db->from('microirrigationbatch b');
        $this->db->join('microirrigationproduct p','p.id = b.pid');
        $this->db->join('microirrigationbatchserial bs','bs.batch_id = b.id');
    ...
    //Datatable View
     function dtlyBatch(){
        ...
        $exp=$this->db->select('id, c_id, pid, p_code, batch_no, s_no_qty')
         ->from('microirrigationbatch2324')->where(array('c_id'=>$_SESSION['comid'], 'pid'=>$pid, 'status'=>'1'))
        //  ->order_by("id", "desc")
        ...
        foreach ($exp->result() as $row) {
            ...
            $actionButton = '
              <ul class="list-inline">  
                <li class="list-inline-item"><a class="btn text-info btn-xs" role="button" href="mlybatchview?id='.$row->id.'" data-toggle="tooltip" title="View Batch Details"> <span class="fa fa-eye"></span></a></li>
            </ul>';
            $output[] = array(
                $i++, 
    ...
    function dtlySerial(){
        ...
        $exp=$this->db->select('sbs.sbsid, sbs.batch_id, sbs.serialno, sbs.cid, sbs.alias, sbs.api_sent, sb.pid, sb.batch_no, pr.p_name')
         ->from('microirrigationbatchserial2324 sbs')->where(array('sbs.cid'=>$_SESSION['comid'], 'sbs.batch_id'=>$bid, 'sb.id'=>$bid, 'sb.c_id'=>$_SESSION['comid']))
         ->join( 'microirrigationbatch2324 sb', 'sb.id = sbs.batch_id' , 'left' )
         ->join( 'microirrigationproduct pr', 'pr.id = sb.pid' , 'left' )
