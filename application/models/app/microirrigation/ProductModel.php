...
class ProductModel extends CI_Model
{
    public function __construct(){
        ...
        // Set table name
        $this->table = 'microirrigationproduct';
    ...
    public function getRows($postData){
        ...
        if($postData['length'] != -1){
            $this->db->where(array('c_id'=>$this->cid,'status'=>'1','is_private'=>'0'));
    ...
    function addProduct(){
        ...
        $spacingcm = $this->input->post('spacingcm');
        $diameter = $this->input->post('diameter');
        $wallthick = $this->input->post('wallthick');
        $netquantity = $this->input->post('netquantity');
        ...
        'uomid'=>$uomid,'net_w'=>$net_w, 'cml'=>$cml, 'cmlbis'=>$cmlbis, 'licno'=>$licno,
        'spacingcm'=>$spacingcm, 'diameter'=>$diameter, 'wallthick'=>$wallthick, 'netquantity'=>$netquantity, 
        'discharge'=>$discharge, 'usp'=>$usp, 'mrp'=>$mrp, 'website'=>$website,
        'status'=>$status,
    ...
    function getPrdinfo(){
        ...
        uomid, net_w, cml, cmlbis, licno')
                ->from('microirrigationproduct')->where(array('id'=> $id, 'c_id' => $_SESSION['comid']))->get();
    ...
    public function getProduct($c_id){
        ...
        $this->db->from('microirrigationproduct');
    ...
    public function update_product_api($id){
        ...
        $this->db->where('id',$id);
        $this->db->update('microirrigationproduct',$data);
    }
    ...
    function dtlyProduct(){
        ...
        if($_SESSION['role']!=$center){
            ...
            $exp=$this->db->select('id, c_id, itemcategory, p_code, p_name, api_sent,onlyprimary')
            ->from('microirrigationproduct')->where(array('c_id'=>$cid, 'status'=>'1'))
            ->order_by("p_code", "asc")
            ->get();
        }
        ...
        foreach ($exp->result() as $row) {            
            ...
            $binfo = $this->db->select('*')->from('microirrigationbatch2324')->where(array('pid'=> $row->id, 'c_id'=>$_SESSION['comid']))->get();
            ...
            $actionButton = '
              <ul class="list-inline">  
                <li class="list-inline-item"><a class="btn text-info btn-xs" role="button" href="mlyproductview?id='.$row->id.'"> <span class="fa fa-eye"></span></a></li>               
              </ul>';
