...
class SecondaryserialModel extends CI_Model
{
    ...
    function dtContainer(){
        ...
        FROM `microirrigationcontainer` as `sc` LEFT JOIN `microirrigationbatchserial` as `sbs` ON `sbs`.`ucc_id` = `sc`.`ctid` 
    ...
    function showucc(){ 
        ...
        $invno=$this->db->query("select ucc from microirrigationcontainer where c_id=? and ucc not like '180089%' order by ucc desc", $_SESSION['comid']);
        $result=$invno->row();
    ...
    function getSerial(){
        $depart=$this->db->order_by("serialno", "asc")->where('cid', $_SESSION['comid'])->get('microirrigationbatchserial'); 
        return $depart;
    }

    public function get_container_maxalias(){
        $this->db->select_max('alias');
        $this->db->from('microirrigationcontainer');
        $res = $this->db->get()->row();
        return $res->alias;
    }
    ...
    if($s_no_qty > $ctotal){
        ...
        $ma =$this->db->select('max(alias) as malias')->from('microirrigationcontainer')->get();
        ...
        if($datat){
            $result = $this->db->insert_batch('microirrigationcontainer',$datat);
        }
            ...
            $total_serialno=sizeof($serial_no);
                ...
                foreach($serial_no as $ser_no){
                    ...
                    if($batch_increment==1000 or $array_key+1==$total_serialno){
                        ...
                        if($updateArr){
    $result1 = $this->db->update_batch('microirrigationbatchserial',$updateArr,'serialno');
    }
                        $updateArr=array();
                    }
    ...
    public function getContainer($bid){
        ...
        //$this->db->join('microirrigationbatchserial sbs','sbs.batch_id=sc.batchno');
        $this->db->where('sc.api_sent','0');
       // $this->db->where('ctid','17');
       //$this->db->where('sc.batchno','1181');
        $this->db->where('sc.batchno',$bid);
        $this->db->from('microirrigationcontainer sc');
    ...
    public function update_container_api($ucc){
        ...
        $this->db->update('microirrigationcontainer',$data);
    }
    ...
    public function containerExport($id,$comid){
        ...
        $this->db->join('microirrigationbatch bs','bs.id = sc.batchno');
        $this->db->from('microirrigationcontainer sc');
        return $this->db->get()->result();
    }
}