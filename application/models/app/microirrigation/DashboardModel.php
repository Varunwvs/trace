...
class DashboardModel extends CI_Model
{
    ...
    function prdCount(){
        if($_SESSION['level']==='3' && $_SESSION['comcat']==="3" && 
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cp = $this->db->select('*')->from('microirrigationproduct')
    ...
    function bthCount(){
        if($_SESSION['level']==='3' && $_SESSION['comcat']==="3" && 
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cp = $this->db->select('*')->from('microirrigationbatch')
            ->where(array('c_id'=> $_SESSION['comid'], 'status'=>'1'))->get();
            $query = $cp->num_rows();
        }else{            
            $cp = $this->db->select('*')->from('microirrigationbatch')
    ...
    function cthCount(){
        if($_SESSION['level']==='3' && $_SESSION['comcat']==="3" && 
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cp = $this->db->select('*')->from('microirrigationbatchserial')
            ->where(array('cid'=> $_SESSION['comid']))->get();
            $query = $cp->num_rows();
        }else{            
            $cp = $this->db->select('*')->from('microirrigationcontainer')
            ->where(array('cid'=> $_SESSION['comid']))->get();
            $query = $cp->num_rows();
        }
        return $query;
    ...
    function asCount(){
        ...
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cid = $_SESSION['comid'];
            $qq = $this->db->select('qr_quantity')->from('users')
            ->where(array('id'=> $cid, 'status'=>'1'))->get();
            foreach($qq->result() as $qrow){
            ...
            $cp = $this->db->select('qr_quantity')->from('users')
            ->where(array('id'=> $cid, 'status'=>'1'))->get();
            $query = $qr_quantity;
        }
        return $query;
