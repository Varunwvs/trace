...
/**
 * Fertilizer Secondary Serial Model
 */
class SecondaryserialModel extends CI_Model
{
    ...
    function dtContainer(){
        ...
        FROM `fertilizercontainer` as `sc` LEFT JOIN `fertilizerbatchserial` as `sbs` ON `sbs`.`ucc_id` = `sc`.`ctid` 
    ...
    function getBinfo(){
        ...
        $bnfo=$this->db->select('id, pid')
                ->from('fertilizerbatch')
                ->where('batch_no', $id)->get();
        return $bnfo->row_array();
    }

    function getSerial(){
        $depart=$this->db->order_by("serialno", "asc")->where('cid', $_SESSION['comid'])->get('fertilizerbatchserial'); 
        return $depart;
    }

    public function get_container_maxalias(){
        $this->db->select_max('alias');
        $this->db->from('fertilizercontainer');
        $res = $this->db->get()->row();
        return $res->alias;
     }
    ...
    if($s_no_qty > $ctotal){
        ...
        if($datat){
            $result = $this->db->insert_batch('fertilizercontainer',$datat);
        }
            ...
            $total_serialno=sizeof($serial_no);
                ...
                foreach($serial_no as $ser_no){
                    ...
                    if($batch_increment==1000 or $array_key+1==$total_serialno){
                        ...
                        if($updateArr){
    $result1 = $this->db->update_batch('fertilizerbatchserial',$updateArr,'serialno');
    }
                        $updateArr=array();
                    }
    ...
    public function update_container_api($ucc){
        ...
        $this->db->update('fertilizercontainer',$data);
    }
    ...
    public function containerExport($id,$comid){
        ...
        $this->db->join('fertilizerbatch bs','bs.id = sc.batchno');
        $this->db->from('fertilizercontainer sc');
        return $this->db->get()->result();
    }
}