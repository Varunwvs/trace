...
class DashboardModel extends CI_Model
{
    ...
    function prdCount(){
        if($_SESSION['level']==='3' && ($_SESSION['comcat']==="2" || $_SESSION['comcat']==="6") && 
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cp = $this->db->select('*')->from('fertilizerproduct')
    ...
    function bthCount(){
        if($_SESSION['level']==='3' && ($_SESSION['comcat']==="2" || $_SESSION['comcat']==="6") && 
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cp = $this->db->select('*')->from('fertilizerbatch')
            ->where(array('c_id'=> $_SESSION['comid'], 'status'=>'1'))->get();
            $query = $cp->num_rows();
        }else{            
            $cp = $this->db->select('*')->from('fertilizerbatch')
    ...
    function cthCount(){
        if($_SESSION['level']==='3' && ($_SESSION['comcat']==="2" || $_SESSION['comcat']==="6") && 
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cp = $this->db->select('*')->from('fertilizerbatchserial')
            ->where(array('cid'=> $_SESSION['comid']))->get();
            $query = $cp->num_rows();
        }else{            
            $cp = $this->db->select('*')->from('fertilizercontainer')
            ->where(array('cid'=> $_SESSION['comid']))->get();
            $query = $cp->num_rows();
        }
        return $query;
