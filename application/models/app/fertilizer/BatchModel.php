...
class BatchModel extends CI_Model
{
    ...
    public function get_maxalias(){
        ...
        $this->db->from('fertilizerbatchserial');
        $res = $this->db->get()->row();
    ...
    function addBatch(){
        ...
        $result = $this->db->insert('fertilizerbatch',$data);
        ...
        if($datat){
            ...
            $results = $this->db->insert_batch('fertilizerbatchserial',$datat);
        }
    ...
    function getBatchinfo(){
        ...
        $pinfo = $this->db->select('id, c_id, pid, batch_no, mfd_date, exp_date')
                ->from('fertilizerbatch')->where(array('id'=> $id, 'c_id' => $_SESSION['comid']))->get();
    ...
    function updBatch(){
        ...
        $result = $this->db->update('fertilizerbatch',$data,array('id'=>$edit_bid));
    ...
    public function update_batch_api($sid){
        ...
        $this->db->update('fertilizerbatchserial',$data);
    }
    
    public function update_batch($bid){
        $data = array('api_sent'=>'1', 'schedule_cron'=>'2');
        $this->db->where('id',$bid);
        $this->db->update('fertilizerbatch',$data);
    }
    ...
    public function label_detail($sno){
       ...
       // $this->db->where('b.id','519');
        $this->db->join('fertilizerproduct p','p.id = b.pid');
        $this->db->join('fertilizerbatchserial bs','bs.batch_id = b.id');
        $this->db->from('fertilizerbatch b');
    ...
    //Datatable View
     function dtlyBatch(){
        ...
        $exp=$this->db->select('id, c_id, pid, p_code, batch_no, s_no_qty, mfd_date, exp_date, status, api_sent,container_status,schedule_cron')
         ->from('fertilizerbatch2324')->where(array('c_id'=>$_SESSION['comid'], 'pid'=>$pid, 'status'=>'1'))
        //  ->order_by("id", "desc")
        ...
        foreach ($exp->result() as $row) {
            ...
            $actionButton = '
              <ul class="list-inline">  
                <li class="list-inline-item"><a class="btn text-info btn-xs" role="button" href="flybatchview?id='.$row->id.'" data-toggle="tooltip" title="View Batch Details"> <span class="fa fa-eye"></span></a></li>
            </ul>';
            $output[] = array(
                $i++, 
    ...
    function dtlySerial(){
        ...
        $exp=$this->db->select('sbs.sbsid, sbs.batch_id, sbs.serialno, sbs.cid, sbs.alias, sbs.api_sent, sb.pid, sb.batch_no, pr.p_name')
         ->from('fertilizerbatchserial2324 sbs')->where(array('sbs.cid'=>$_SESSION['comid'], 'sbs.batch_id'=>$bid, 'sb.id'=>$bid, 'sb.c_id'=>$_SESSION['comid']))
         ->join( 'fertilizerbatch2324 sb', 'sb.id = sbs.batch_id' , 'left' )
         ->join( 'fertilizerproduct pr', 'pr.id = sb.pid' , 'left' )
