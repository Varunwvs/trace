...
/**
 * pesticide Secondary Serial Model
 */
class SecondaryserialModel extends CI_Model
{
    ...
    function dtContainer(){
        ...
        $query = $this->db->query("SET SESSION group_concat_max_len = 2048");
        $sql="SELECT `sc`.`ctid`, `sc`.`pid`, `sc`.`ucc`, `sc`.`batchno`, `sc`.`serialno`, 
        `sc`.`alias`, GROUP_CONCAT('0110', `sbs`.`alias` SEPARATOR ', ') as palias
        FROM `pesticidecontainer` as `sc` LEFT JOIN `pesticidebatchserial` as `sbs` ON `sbs`.`ucc_id` = `sc`.`ctid` 
    ...
    function showucc(){ 
        ...
        $invno=$this->db->query("select ucc from pesticidecontainer where c_id=? and ucc not like '180089%' order by ucc desc", $_SESSION['comid']);
        $result=$invno->row();
    ...
    function getSerial(){
        $depart=$this->db->order_by("serialno", "asc")->where('cid', $_SESSION['comid'])->get('pesticidebatchserial'); 
        return $depart;
    }

    public function get_container_maxalias(){
        $this->db->select_max('alias');
        $this->db->from('pesticidecontainer');
        $res = $this->db->get()->row();
        return $res->alias;
    }
    ...
    if($s_no_qty > $ctotal){
        ...
        $ma =$this->db->select('max(alias) as malias')->from('pesticidecontainer')->get();
        ...
        if($datat){
            $result = $this->db->insert_batch('pesticidecontainer',$datat);
        }
            ...
            $total_serialno=sizeof($serial_no);
                ...
                foreach($serial_no as $ser_no){
                    ...
                    if($batch_increment==1000 or $array_key+1==$total_serialno){
                        ...
                        if($updateArr){
    $result1 = $this->db->update_batch('pesticidebatchserial',$updateArr,'serialno');
    }
                        $updateArr=array();
                    }
    ...
    public function update_container_api($ucc){
        ...
        $this->db->update('pesticidecontainer',$data);
    }
    ...
    public function containerExport($id,$comid){
        ...
        $this->db->join('pesticidebatch bs','bs.id = sc.batchno');
        $this->db->from('pesticidecontainer sc');
        return $this->db->get()->result();
    }
}