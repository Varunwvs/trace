...
class DashboardModel extends CI_Model
{
    ...
    function prdCount(){
        if($_SESSION['level']==='3' && $_SESSION['comcat']==="5" && 
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cp = $this->db->select('*')->from('pesticideproduct')
    ...
    function bthCount(){
        if($_SESSION['level']==='3' && $_SESSION['comcat']==="5" && 
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cp = $this->db->select('*')->from('pesticidebatch')
            ->where(array('c_id'=> $_SESSION['comid'], 'status'=>'1'))->get();
            $query = $cp->num_rows();
        }else{            
            $cp = $this->db->select('*')->from('pesticidebatch')
    ...
    function cthCount(){
        if($_SESSION['level']==='3' && $_SESSION['comcat']==="5" && 
        ($_SESSION['role']==='company' || $_SESSION['role']==='rcholder')){
            $cp = $this->db->select('*')->from('pesticidebatchserial')
            ->where(array('cid'=> $_SESSION['comid']))->get();
            $query = $cp->num_rows();
        }else{            
            $cp = $this->db->select('*')->from('pesticidecontainer')
            ->where(array('cid'=> $_SESSION['comid']))->get();
            $query = $cp->num_rows();
        }
        return $query;
