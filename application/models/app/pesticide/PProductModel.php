...
class PProductModel extends CI_Model
{
    ...
    public function getRows($postData){
        ...
        if($postData['length'] != -1){
            $this->db->where(array('c_id'=>$_SESSION['comid'],'status'=>'1','is_private'=>'1', 'applicationid'=>'PS'));
    ...
    function pshowupc(){ 
        $value2 = '';
        $inv = 'SYN'."000";
        $invno = $this->db->select('max(p_code) as p_code')
                ->from('publicproduct')
                ->get();
    ...
    function addPProduct(){
        ...
        if(isset($_FILES["leaflet"]["type"]))
        {
            ...
            'uomid'=>$uomid,'net_w'=>$net_w, 'formulation'=>$formulation, 'cibno'=>$cibno, 'mlno'=>$mlno, 'antst'=>$antst, 'caution'=>$cs,'status'=>$status,
            'api_sent'=>'0', 'api_response'=>'', 'onlyprimary'=>$onlyprimary, 'is_private'=>'1', 'leaflet'=>$image);
    ...
    function addPPProduct($data){
        // Insert the data
        if ($this->db->insert_batch('publicproduct', $data)) {
            return true;
        }
        return false;
    }
