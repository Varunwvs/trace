...
class ProductModel extends CI_Model
{
    ...
    function dtlyProduct(){
        ...
        if($_SESSION['role']!=$center){
            ...
            ->from('pesticideproduct')->where(array('c_id'=>$_SESSION['comid'], 'status'=>'1'))
            ->order_by("p_code", "asc")
            ->get();
        }else{
            $exp=$this->db->select('id, c_id, itemcategory, p_code, p_name, api_sent,onlyprimary')
            ->from('pesticideproduct')->where(array('c_id'=>$cid, 'status'=>'1'))
            ->order_by("p_code", "asc")
            ->get();
        }
        ...
        foreach ($exp->result() as $row) {            
            ...
            $binfo = $this->db->select('*')->from('pesticidebatch2324')->where(array('pid'=> $row->id, 'c_id'=>$_SESSION['comid']))->get();
            ...
            $actionButton = '
              <ul class="list-inline">  
                <li class="list-inline-item"><a class="btn text-info btn-xs" role="button" href="plyproductview?id='.$row->id.'"> <span class="fa fa-eye"></span></a></li>               
              </ul>';
