...
class MIBulkImport extends CI_Controller {
  public function __construct() {
    ...
    if(empty($this->session->userdata('memlog'))){
      ...
      $this->load->model('app/microirrigation/SecondaryserialModel','ccm');
    }
    
    public function sample_csv(){
        $filename = 'uploads/microirrigation_products.csv';
    ...
    function import() {
        if (isset($_FILES["file"])) {
            ...
            $handle = fopen($filename, "r");
                ...
                $keys = array('name', 'marketed_by', 'p_category', 'sub_category', 'p_code', 'p_name','b_name','unit_w','net_w',onlyprimary);
//var_dump($keys);exit();
                $final = array();
     ...
     //print_r($file);exit();
            $header = array("Product Name","UPC","Category","Sub Category","Batch Number","Serial Number","Alias Code");
            fputcsv($file, $header);
            ...
            if($batch_id > 0){
                ...
                if($serialnoData){
                    foreach($serialnoData as $s_data ){
                        ...
                        fputcsv($file,array($p_name,'="'.$p_code.'"',$p_category.'"',$sub_category,$batch_no,'="'.$s_data->serialno.'"','="'.$alias.'"')); 
                    }
                }else{
    ...
    public function pbatch_export(){
        ...
        if($batchData){
            foreach($batchData as $b_data){
                ...
                $url = base_url().'sb/'.$comid.'/'.$pid.'/'.$p_code.'/'.$batch_id.'?11=0&12=0';
                // $url = base_url().'sb/'.$comid.'/'.$pid.'/'.$p_code.'/'.$bid.'?11='.$srrow['alias'].'&12='.$srrow['alias'];
            }            
        }else{
            $p_name = " ";
            $p_code = " ";
        ...
        $file = fopen('php://output', 'w');
        //print_r($file);exit();
            $header = array("Product Name","UPC","Category","Sub Category","Batch Number","Serial Number","Mfg Date","Exp Date", "URL");
            fputcsv($file, $header);
            ...
            if($batch_id > 0){
                ...
                if($serialnoData){
                    foreach($serialnoData as $s_data ){
                        fputcsv($file,array($p_name,$p_code,$p_category,$sub_category,$batch_no,$s_data->serialno,$mfg_date,$exp_date,$url)); 
                    }
                }else{
                    $this->session->set_flashdata('message', 'Something went wrong..');
                    redirect('productmanage');
