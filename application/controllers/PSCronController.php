...
class PSCronController extends CI_Controller {
	...
	public function process_batch_containers(){
	   ...
	   $query = $this->db->query("select id, (select count(ctid) from pesticidecontainer where batchno=sb.id and cron_status=1) as pending_containers from pesticidebatch sb where schedule_cron=1 ORDER BY sb.order_by DESC");
	   $seedbatch=$query->row_array();
	   
	   echo json_encode($seedbatch); exit;
	   ...
	   if(@$seedbatch['id']>0){
	       ...
	       if($seedbatch['pending_containers']>0){
	           ...
	           $query = $this->db->query("select ctid from pesticidecontainer sc where cron_status=1 and batchno=?", $seedbatch['id']);
	           ...
	           if($seedcontainer){
	            ...
	            $this->db->update("pesticidebatch", array("schedule_cron"=>2), array("id"=>$seedbatch['id']));
	...
	// fetch container
	 ...
	 $query = $this->db->query("select * from pesticidecontainer sc where ctid=?", $container_id);
     $container=$query->row();
     
     //echo json_encode($container); exit();
            ...
            curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, 0);
		 	...
		 	if ($err) {
		 		...
		 		//update container cron status = 2
                    ...
                    $this->db->update("pesticidecontainer", array("cron_status"=>2), array("ctid"=>$container->ctid));
		 	}
	...
	public function process_batch_primary(){
	    ...
	    $query = $this->db->query("select id,batch_no,mfd_date,exp_date,p_code from pesticidebatch sb where schedule_cron=1 ORDER BY sb.order_by ASC");
	   $seedbatch=$query->row_array();
					...
					if ($err) {
						...
						foreach($sbsid as $sid){
				 		...
				 		if($resp_len == '0'){
				 			...
				 			$this->db->update("pesticidebatch", array("schedule_cron"=>2), array("id"=>$batch_id));
				 		}else{
				 		    $this->db->update("pesticidebatch", array("schedule_cron"=>3), array("id"=>$batch_id));
				 		}
					}
