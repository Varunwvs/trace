...
class MIApiController extends CI_Controller {
	...
	public function api(){
	    ...
	    $this->load->datatable_template('api/miapiPage', $data);
	}
...
// Sending Batch data 	
    ...
    public function get_product(){		
		...
		if($products){
			foreach($products as $prod){
				...
				$licno = $prod->licno;
				$gsm = '';
				$this->saveProductMaster($applicationid,$productCode,$manufacturerName,$supplierName,$ItemCategoryID,$categoryName,$SubCategoryID,$subCategoryName,$ItemID,$productName,$brandName,$uomId,$weight,$cml,$cmlbis,$licno,$gsm,$id);
			}
		}
	}

	public function saveProductMaster($applicationid,$productCode,$manufacturerName,$supplierName,$ItemCategoryID,$categoryName,$SubCategoryID,$subCategoryName,$ItemID,$productName,$brandName,$uomId,$weight,$cml,$cmlbis,$licno,$gsm,$id){
	    $response = '';	    
		...
		$postData = array(
			...
			"MarketedBy"=>$supplierName,
			"CIBRegistrationCertificate"=>$cml,
			"LicenseNumber"=>$licno,
	...
	public function get_batch(){
		...
		$access_token = $this->generate_token();
		$auth_key = 'Bearer '.$access_token;
		$saveQRDetail_url = get_settings("saveQRDetailMI_url");
		if($batches){
			foreach($batches as $b){
				$productCode = $b->p_code;
				$bundlelateralsize = $b->net_w;
				...
				foreach($batch_serialno as $bs){
			    	...
			    	'ProductCode' => $productCode,
			    	'BundleLateralSize' => $bundlelateralsize,
