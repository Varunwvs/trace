...
$(document).ready(function(){
    ...
    $(document).on('click','#submit',function(e){
        ...
        $("#formProduct").unbind('submit').bind('submit', function() {
            ...
            // }
            if(cname && marketed_by && p_category && sub_category && upc && p_name && b_name && unit_w && net_w && cibno) {
                var form = $("#formProduct");
                var formData = new FormData(form[0]);
                $.ajax({
                    url:"SeedController/addPProduct",   
                    method: "POST",
                    data: formData,
                    contentType: false,
                    cache: false,
