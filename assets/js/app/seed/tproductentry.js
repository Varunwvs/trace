    ...
    $.ajax({
        ...
        success: function(response){   
          ...
          $.each(response,function(index,data){
            // console.log(data['sub_name']+'ghfjhfh' + data['id'])
            $('#sub_category').append('<option value="'+data['id']+'|'+data['sub_name']+'">'+data['sub_name']+'</option>');
          });
        }
    });
    return false;
...
//   });
  ...
  $("#formProduct").unbind('submit').bind('submit', function(e) {
    ...
    if(cname && marketed_by && p_category && sub_category && upc && p_name && b_name && unit_w && net_w) {
      ...
      $.ajax({
        url: "SeedController/taddProduct",
        method: "POST",
